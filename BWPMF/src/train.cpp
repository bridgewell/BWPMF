...
SEXP train(SEXP Rmodel, SEXP Rtraining_history, SEXP Rtesting_history, SEXP Ritem_inverted_index, int iteration) {
  ...
  static std::vector<double> item_sum;
  static std::vector<double> user_sum;
  NumericVector training_logloss(iteration);
  NumericVector testing_logloss(iteration);
  double retval;
#pragma omp parallel
  {
    std::vector<double> local_item_sum(model.K, 0.0);
    ...
    for(int iteration_i = 0;iteration_i < iteration;iteration_i++) {
      // phi
#pragma omp for
      for(size_t i = 0;i < training_history.userdata.size();i++) {
        for(size_t j = 0;j < training_history.userdata[i].size();j++) {
      ...
      // user
      const double a1 = model.prior.a1, a2 = model.prior.a2, b2 = model.prior.b2;
      const double c1 = model.prior.c1, c2 = model.prior.c2, d2 = model.prior.d2;
#pragma omp single 
      {
        item_sum.resize(model.K);
        std::fill(item_sum.begin(), item_sum.end(), 0.0);
...
#pragma omp for
      for(size_t user = 0;user < model.user_param.size();user++) {
        const auto& user_data(training_history.userdata[user]);
        const auto& user_param(model.user_param[user]);
...
#pragma omp once
      {
        ...
        training_logloss[iteration_i] = retval;
      }
      // testing logloss
#pragma omp once
      {
...
#pragma omp for
      for(size_t user = 0;user < model.user_param.size();user++) {
        const auto& user_data(testing_history.userdata[user]);
        const auto& user_param(model.user_param[user]);
        for(const auto& element : user_data) {
          const size_t item = element.first;
          const int y = element.second;
...
#pragma omp once
      {
        ...
        testing_logloss[iteration_i] = retval;
      }
    } // iteration
  }
  return List::create(
    Named("training_logloss") = training_logloss,
    Named("testing_logloss") = testing_logloss
  );
}
